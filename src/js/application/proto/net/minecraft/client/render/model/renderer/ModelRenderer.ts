...
export default class ModelRenderer {
    ...
    private textureOffsetX: number;
    private textureOffsetY: number;
    public rotateAngleX: number;
    public rotateAngleY: number;
    public rotateAngleZ: number;
    public rotationPointX: number;
    public rotationPointY: number;
    public rotationPointZ: number;
    private scaleX: number;
    private scaleY: number;
    private scaleZ: number;
    private cubes: Polygon[][];
    private children: ModelRenderer[];
    private bone: THREE.Object3D;

    /**
     * Create cube object
     */
    constructor(name: string, textureWidth: number, textureHeight: number) {
        this.name = name;
     ...
     */
    setTextureOffset(textureOffsetX: number, textureOffsetY: number) {
     ...
     * @param z Rotation z
     */
    setRotationAngle(x: number, y: number, z: number) {
        this.rotateAngleX = x;
        this.rotateAngleY = y;
        this.rotateAngleZ = z;
        return this;
     ...
     */
    addBox(offsetX: number, offsetY: number, offsetZ: number, width: number, height: number, depth: number, inflate: number = 0, mirror: boolean = false) {
    ...
    addChild(model: ModelRenderer) {
        this.children.push(model);
    }

    removeChild(model: ModelRenderer) {
    ...
    getModelByName(name: string): ModelRenderer {
        if (this.name === name) {
            return this;
        }
    ...
    rebuild(tessellator: Tessellator, group: THREE.Object3D) {
        // Clear meshes
        this.bone.clear();
    ...
    copyTransformOf(modelRenderer: ModelRenderer) {
