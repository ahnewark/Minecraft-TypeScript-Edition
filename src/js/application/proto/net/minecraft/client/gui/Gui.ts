...
export default class Gui {
    ...
    drawRect(stack: CanvasRenderingContext2D, left: number, top: number, right: number, bottom: number, color: number, alpha = 1) {
        stack.save();
    ...
    drawGradientRect(stack: CanvasRenderingContext2D, left, top, right, bottom, color1, color2) {
    ...
    drawTexture(stack: CanvasRenderingContext2D, texture, x, y, width, height, alpha = 1.0) {
        Gui.drawSprite(stack, texture, 0, 0, 256, 256, x, y, width, height, alpha);
    }

    drawSprite(stack: CanvasRenderingContext2D, texture, spriteX, spriteY, spriteWidth, spriteHeight, x, y, width, height, alpha = 1.0) {
        Gui.drawSprite(stack, texture, spriteX, spriteY, spriteWidth, spriteHeight, x, y, width, height, alpha);
    }

    drawBackground(stack: CanvasRenderingContext2D, texture, width, height, scale = 2) {
    ...
    renderBlock(stack: CanvasRenderingContext2D, texture, block, x, y) {
        let scale = 0.18;
    ...
    renderBlockFace(stack: CanvasRenderingContext2D, texture, block, triangles, face) {
        // UV Mapping
    ...
    static drawSprite(stack: CanvasRenderingContext2D, texture, spriteX, spriteY, spriteWidth, spriteHeight, x, y, width, height, alpha = 1.0) {
        stack.save();
        stack.globalAlpha = alpha;
        stack.drawImage(
            texture,
