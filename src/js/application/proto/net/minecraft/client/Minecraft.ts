...
export default class Minecraft {
    ...
    public resources: { [texturePath: string]: HTMLImageElement };
    public currentScreen: GuiScreen;
    ...
    async loadWorld(world) {
        if (world === null) {
            ...
            await this.world.spawnPlayer(this.player);
    ...
    async onRender(partialTicks) {
        if (this.isInGame()) {
            ...
            // Update lights
            while (await this.world.lighting_func_6465_g()) {
                // Empty
            }
    ...
    async onTick() {
        ...
        if (this.loadingScreen !== null && this.isInGame()) {
            let cameraChunkX = Math.floor(this.player.posX) >> 4;
            let cameraChunkZ = Math.floor(this.player.posZ) >> 4;
    ...
    onKeyPressed(button) {
        ...
        for (let i = 1; i <= 9; i++) {
            if (button === 'Digit' + i) {
                this.player.inventory.setCurrentItem(i - 1, false);
            }
        }
    ...
    async onMouseClicked(button) {
        if (this.window.isLocked()) {
            let hitResult = await this.player.rayTrace(5, this.timer.partialTicks);

            // Destroy block
            if (button === 0) {
                if (hitResult != null) {
                    // Get previous block
                    let typeId = await this.world.getBlockId(hitResult.blockX, hitResult.blockY, hitResult.blockZ);
                    let block = Block.blocksList[typeId];

                    if (typeId !== 0) {
                        let soundName = block.stepSound.func_1146_a();

                        // Play sound
                        this.soundManager.playSound(
                            soundName,
                            hitResult.blockX + 0.5,
                            hitResult.blockY + 0.5,
                            hitResult.blockZ + 0.5,
                            1.0,
                            1.0
                        );

                        // Spawn particle
                        this.particleRenderer.spawnBlockBreakParticle(this.world, hitResult.blockX, hitResult.blockY, hitResult.blockZ);

                        // Destroy block
                        await this.world.setBlock(hitResult.blockX, hitResult.blockY, hitResult.blockZ, 0);
                    }
                }
            ...
            // if (button === 2) {
            //     if (hitResult != null) {
            //         let x = hitResult.blockX + hitResult.face.x;
    ...
    onMouseScroll(delta) {
        if (this.isInGame()) {
            this.player.inventory.changeCurrentItem(delta);
        }
    }
