...
export  class WorldGenHellLava extends WorldGenerator {
	...
	public async generate(world1: World| null, random2: Random | null, i3: int, i4: int, i5: int):  Promise<boolean> {
		if(await world1.getBlockId(i3, i4 + 1, i5) !== BlockRegistry.bloodStone.blockID) {
			return false;
		} else if(await world1.getBlockId(i3, i4, i5) !== 0 && await world1.getBlockId(i3, i4, i5) !== BlockRegistry.bloodStone.blockID) {
			return false;
		} else {
			let  i6: int = 0;
			if(await world1.getBlockId(i3 - 1, i4, i5) === BlockRegistry.bloodStone.blockID) {
				++i6;
			}
			...
			if(await world1.getBlockId(i3, i4, i5 - 1) === BlockRegistry.bloodStone.blockID) {
				++i6;
			}
